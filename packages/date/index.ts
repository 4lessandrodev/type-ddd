...
export class Dates extends ValueObject<Date> {
	...
	format(format: DateFormat): string {
		const date = this.addZero(this.props.getDate());
		const fullYear = this.props.getFullYear().toString();
		const month = this.addZero(this.props.getMonth() + 1);
		const hours = this.addZero(this.props.getHours());
		const minutes = this.addZero(this.props.getMinutes());
		const sec = this.addZero(this.props.getSeconds());
	...
	isWeekday(date?: Date): boolean {
		const day = date?.getDay() ?? this.props.getDay();
		return day > 0 && day < 6;
	}
