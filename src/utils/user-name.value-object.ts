...
export class UserNameValueObject extends ValueObject<Prop>{
	...
	public static create(value: string): Result<UserNameValueObject>{
		...
		if (!isValidValue) {
			return Result.fail<UserNameValueObject>('Invalid name length. Must has min 2 and max 40 chars');
		}
		return Result.ok<UserNameValueObject>(new UserNameValueObject({ value }))
	}
}
