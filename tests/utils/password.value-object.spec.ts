import {PasswordValueObject} from '../../src/utils/password.value-object';
...
describe('password.value-object', ()=>{
	...
	it('should fail if try to create a password less than 5 char', ()=>{
		...
		expect(valueObject.errorValue()).toBe('Password must has min 5 and max 21 chars');
	});

	it('should fail if try to create a password greater than 21 char', ()=>{
		const valueObject = PasswordValueObject.create('123456789101112131415161718');
		expect(valueObject.isSuccess).toBe(false)
		expect(valueObject.errorValue()).toBe('Password must has min 5 and max 21 chars');
	});
