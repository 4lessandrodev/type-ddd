...
describe('date?.value-object', () => {
	...
	it('should format date', () => {
		const date = DateValueObject.create(currentDate).value();
		expect(date?.format('DD-MM-YY')).toMatch(
			/[0-9]{2}\-[0-9]{2}\-[0-9]{2}/,
		);
	});
	...
	it('should format date', () => {
		const date = DateValueObject.create(currentDate).value();
		expect(date?.format('YY-MM-DD')).toMatch(
			/[0-9]{2}\-[0-9]{2}\-[0-9]{2}/,
		);
	});
	...
	it('should calculate difference in hours', () => {
		...
		expect(date?.differenceInHours(new Date('2022-01-20 00:00:00'))).toBe(
			3,
		);
	});
	...
	it('should calculate difference in weeks', () => {
		...
		expect(date?.differenceInWeeks(new Date('2022-01-01 00:00:00'))).toBe(
			1,
		);
	});
	...
	it('should calculate difference in years', () => {
		...
		expect(date?.differenceInYears(new Date('2021-01-01 00:00:00'))).toBe(
			1,
		);
	});
